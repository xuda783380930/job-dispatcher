package com.opensource.jobdispatcher.model.middleware.vo;
...
public class JobStructVO{
    ...
    public <T> JobStructVO(List<T> taskTList){
		...
		if(null!=taskTList) {
			List<String> upstreamTaskCodes = null;
			String taskCode = null;
			for(T tT:taskTList) {
				if(tT.getClass().equals(TaskBuilder.class)) {
					 TaskBuilder tb = (TaskBuilder)tT;
					 upstreamTaskCodes =tb.getUpstreamTaskCodes();
					 taskCode = tb.getTaskCode();
				}else if(tT.getClass().equals(Task.class)) {
					 Task t = (Task)tT;
					 upstreamTaskCodes =t.getUpstreamTaskCodes();
					 taskCode = t.getTaskCode();
				}
				if(null!=upstreamTaskCodes&&upstreamTaskCodes.size()>0) {
